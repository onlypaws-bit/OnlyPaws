...
serve(async (req) => {
  ...
  try {
    ...
    if (!["trialing", "active", "past_due", "unpaid"].includes(support.status)) {
      return new Response(JSON.stringify({ error: "Subscription is not resumable" }), {
        status: 409,
        headers: corsHeaders,
      });
    }

    if (support.cancel_at_period_end === false) {
      return new Response(
        JSON.stringify({
          ok: true,
          alreadyResumed: true,
    ...
    if (updateError) {
      console.error("support_us resume update error:", updateError);
    ...
    console.error("support-us-resume error:", error);
